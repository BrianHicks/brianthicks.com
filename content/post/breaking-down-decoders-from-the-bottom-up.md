...
I told you then that it's a good approach because the structure doesn't change too much.
But sometimes it's way too difficult to see the whole structure at once.
Then, you want to let the compiler guide your steps.
...
Hey!
That fits us!
We want to be able to decode our first level, and not worry about the rest.
So let's do that!
...
`value` is a `Decoder Value`, where `Value` is an opaque type (meaning we can't see into it, like a frosted glass window.)
We already know that we have a `Dict` with site names, so let's tell Elm that.
...
This says "we want the keys as strings, but don't particularly care about the values".
When we try to decode, we get exactly what we asked for:
...
Now, you might look at this and go "well, hey, why can't I use those things I can see in `Value`?"
But remember that we haven't told Elm what types they are!
That means the compiler can't check that we're using these in a way that makes sense.
To make a reliable app, we need to attach that information using decoders.
Yet there *is* a benefit to being able to see our remaining structure: it gives us our next step!
...
Now we need one last step: our `machine` decoder from before:
...
2. Use the REPL or tests to verify that what you wrote worked.
3. Refine any remaining `value`s with steps 1 and 2 until you're done!

If you get stuck, you might be trying to tackle too much at once!
